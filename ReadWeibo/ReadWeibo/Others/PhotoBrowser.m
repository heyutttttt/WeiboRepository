...
- (id)initWithActivityIndicatorStyle:(UIActivityIndicatorViewStyle)style
{
    ...
    if (self) {
        UIView* background = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 80, 80)];
        background.backgroundColor = [UIColor blackColor];
        background.layer.cornerRadius = 6;
        background.alpha = 0.6;
        background.center = CGPointMake(self.width / 2, self.height / 2);
...
@interface PhotoBrowser ()<UIScrollViewDelegate, UIGestureRecognizerDelegate, UIActionSheetDelegate>
{
    ...
    NSMutableArray *_activityIndicatorArr;
                   ...
                   delegate:(id<PhotoBrowserDelegate>)delegate
{
    ...
    if (self)
    {
        ...
        [_scrollView setContentSize:CGSizeMake([UIScreen mainScreen].bounds.size.width * [_bigImageInfos count], [UIScreen mainScreen].bounds.size.height)];
        [_scrollView setContentOffset:CGPointMake([UIScreen mainScreen].bounds.size.width * _currentIndex, 0)];
...
- (void)show
{
    ...
    if (originImageView)
    {
        ...
        {
            CGFloat imageHeight = [UIScreen mainScreen].bounds.size.width * originImageView.image.size.height / originImageView.image.size.width;
            scrollImageView.frame = CGRectMake(0, ([UIScreen mainScreen].bounds.size.height - imageHeight) / 2, [UIScreen mainScreen].bounds.size.width, imageHeight);
        }
    }
...
- (void)updateImageViewForIndex:(NSInteger)index image:(UIImage *)image
{
    ...
    scrollView.contentSize = CGSizeMake(imageView.width, MAX(imageView.height, scrollView.height));
}
...
- (void)downloadContentWithIndex:(NSInteger)index
{
    ...
    if(!image)
    {
        [[_activityIndicatorArr objectAtIndex:index] startAnimating];
        [[SDWebImageManager sharedManager] downloadWithURL:url
                                                  ...
                                                  } completed:^(UIImage *aImage, NSError *error, SDImageCacheType cacheType, BOOL finished)
         {
             if (finished)
             {
                 [[_activityIndicatorArr objectAtIndex:index] stopAnimating];
                 if (aImage)
                 {
...
//    [longPressGes setDelegate:self];
    ...
    for (NSInteger i = 0; i <= MAX([_bigImageInfos count] - 1 - _currentIndex, _currentIndex); i++)
    {
...
- (void)initImageView:(int)i
{
    ...
    UIScrollView *imageScrollView=[[UIScrollView alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width *i, 0, ScreenWidth, ScreenHeight)];
    ...
    UIActivityIndicatorView* activityIndicator = [[MyActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
    activityIndicator.center = CGPointMake(ScreenWidth * (i + 0.5), ScreenHeight / 2);
...
- (void)loadImageView:(int)i
{
    ...
    else
    {
        ...
        else if ([smallImageInfo isKindOfClass:[NSString class]])
        {
            ...
            [[_activityIndicatorArr objectAtIndex:i] startAnimating];
            [[SDWebImageManager sharedManager] downloadWithURL:url
                                                       options:SDWebImageLowPriority
                                                      progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                                      ...
                                                      } completed:^(UIImage *aImage, NSError *error, SDImageCacheType cacheType, BOOL finished)
             {
                 if (finished)
                 {
                     ...
                     [[_activityIndicatorArr objectAtIndex:i] stopAnimating];
                     [self downloadContentWithIndex:i];
                 }
             }];
        }
